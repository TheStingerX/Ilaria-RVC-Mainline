...
REM Print the step to the terminal
echo Launching TensorBoard and Ilaria RVC :) ...

REM Launch TensorBoard in the background
start "" /B cmd /c "tensorboard --logdir="%~dp0\logs""

REM Launch the UI in the background
start "" /B cmd /c "%~dp0\runtime\python.exe infer-web.py --pycmd %~dp0\runtime\python.exe --port 7897"

REM Print the step to the terminal
echo Keep the command prompt open...
